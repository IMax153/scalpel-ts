...
describe('StateOption', () => {
  describe('constructors', () => {
    ...
    it('put', () => {
      assert.deepStrictEqual(pipe(_.put(1), _.execute(2)), 1)
    })

    it('modify', () => {
      const double = (n: number) => n * 2
      assert.deepStrictEqual(pipe(_.modify(double), _.execute(1)), 2)
    })
  ...
  describe('utils', () => {
    ...
    it('execute', () => {
      assert.deepStrictEqual(pipe(_.none as _.StateOption<string, number>, _.execute('a')), 'a')
      assert.deepStrictEqual(pipe(_.some(1) as _.StateOption<string, number>, _.execute('a')), 'a')
    })
  })
